...
Per ogni record hai:
- IMMAGINI da analizzare (le PRIME immagini che ti vengono fornite).
- IMMAGINI simili recuperate dal database (le SUCCESSIVE immagini).
- Le informazioni testuali delle immagini simili (riportate qui sotto).

Testi delle immagini simili:
{retrieved_docs}

Devi:
1) Proporre la TUA classificazione indipendente usando le immagini simili come riferimento per migliorare l'accuratezza.
2) Spiegare brevemente il tuo ragionamento.
...
{{
  ...
  "notes": "La dimensione è medium perchè nelle immagini è presente una busta di plastica di immondizia piena di giornali e altre immagini contenenti rifiuti simili sono state classificate come medium.",
